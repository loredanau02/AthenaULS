...
public class AuthService {
    ...
    public Student authenticateStudent(String studentId, String password) {
        try {
            ...
            String salt = authDoc.getString("salt");
     ...
     * @param password Password
     * @return Librarian object if authentication is successful, null otherwise
     */
    public Librarian authenticateLibrarian(String staffId, String password) {
        try {
            Firestore db = FirebaseConfig.getFirestoreInstance();

            // First, find the librarian by staffId
            Query query = db.collection(LIBRARIANS_COLLECTION).whereEqualTo("staffId", staffId);
            ApiFuture<QuerySnapshot> queryFuture = query.get();
            QuerySnapshot querySnapshot = queryFuture.get();

            if (querySnapshot.isEmpty()) {
                System.err.println("Librarian not found with Staff ID: " + staffId);
                return null;
            }

            // Get the librarian document
            DocumentSnapshot librarianDoc = querySnapshot.getDocuments().get(0);
            String systemId = librarianDoc.getId();
            ...
            if (!authDoc.exists()) {
                System.err.println("Auth record not found for librarian: " + staffId);
                return null;
            }
            ...
            if (storedHash.equals(computedHash)) {
                // Authentication successful
                Librarian librarian = librarianDoc.toObject(Librarian.class);
                ...
                System.err.println("Invalid password for librarian: " + staffId);
                return null;
            }
        } catch (InterruptedException | ExecutionException e) {
            System.err.println("Error authenticating librarian: " + e.getMessage());
            return null;
        }
    }
    ...
    public boolean updatePassword(String userId, String oldPassword, String newPassword) {
        try {
            ...
            String computedHash = hashPassword(oldPassword, salt);
